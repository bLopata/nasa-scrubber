...
const getImageUrlsFromResponse = res => {
  ...
  const preview_html = $(
    "#content > div:nth-child(1) > table:nth-child(5) > tbody:nth-child(1) > tr:nth-child(1) > td:nth-child(1) > input:nth-child(7)"
  ).val();
...
async function getPageUrls(webPage) {
  const a = await axios.get(webPage);
  ...
  const pageUrls = Array(image_count)
    .fill()
    .map((value, i) => urlOf(i));
  return pageUrls;
}
...
async function getImageUrls(pageUrls) {
  const allPromises = pageUrls.map(url => axios.get(url));
...
async function main() {
  try {
    const pageUrls = await getPageUrls(
      "http://superstarfloraluk.com/8522767-NASA-Galaxy-Pictures-High-Resolution.html"
    );
